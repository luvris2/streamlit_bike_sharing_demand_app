...
def run_eda() :
    ...
    axes[0].set(xlabel='계절(1:봄, 2:여름, 3:가을, 4:겨울)', ylabel='대여량',title="계절별 대여량")
    axes[1].set(xlabel='휴일/평일(0:휴일, 1:평일)', ylabel='대여량',title="휴일/평일별 대여량")
    st.pyplot(fig)
    ...
    ax2.set_xlabel("휴일/평일(0:휴일, 1:평일)", fontsize=20)
    ...
    ax3.set_xlabel("요일(1:월, 2:화, 3:수, 4:목, 5:금, 6:토, 7:일)", fontsize=20)
    ...
    ax4.set_xlabel("날씨(1:맑음, 2:흐림, 3:이슬비, 4:호우))", fontsize=20)
    ...
    ax5.set_xlabel("계절(1:봄, 2:여름, 3:가을, 4:겨울)", fontsize=20)
    ax5.set_ylabel("대여량", fontsize=20)
